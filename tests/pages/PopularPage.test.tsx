...
import { setPopularVideos } from "../mocks/db";
...
describe("PopularPage", () => {
    ...
    describe("video grid", () => {
        it("should render video grid with videos from popular", async () => {
        ...
        it("should redirect to the video page with videos from popular when an item is clicked", async () => {
            createVideos(3, {}); // other videos
            const videos = createVideos(3, {});
            setPopularVideos(videos);
            const {
...
function navigateToPage() {
    const { getLocation } = navigateTo("/popular");
