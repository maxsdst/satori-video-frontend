...
import { setLatestVideos } from "../mocks/db";
...
describe("LatestPage", () => {
    ...
    describe("video grid", () => {
        it("should render video grid with videos from latest", async () => {
        ...
        it("should redirect to the video page with videos from latest when an item is clicked", async () => {
            createVideos(3, {}); // other videos
            const videos = createVideos(3, {});
            setLatestVideos(videos);
            const {
...
function navigateToPage() {
    const { getLocation } = navigateTo("/latest");
